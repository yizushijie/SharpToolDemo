...
namespace Harry.LabTools.LabMcuFunc
{
	...
	/// HVPP编程使用的枚举变量
	/// </summary>
	public enum CMCUFUNC_CMD_HVPP : byte 
	{
		CMD_HVPP_OPEN_CLOSE		=0x30	,
		CMD_HVPP_ERASE					,
		CMD_HVPP_FLASH_PAGE_READ		,
		CMD_HVPP_FLASH_PAGE_WRITE		,
		CMD_HVPP_EEPROM_PAGE_READ		,
		CMD_HVPP_EEPROM_PAGE_WRITE		,
		CMD_HVPP_FUSE_LOCK_READ			,
		CMD_HVPP_FUSE_WRITE				,
		CMD_HVPP_LOCK_WRITE				,
		CMD_HVPP_ID_READ				,
		CMD_HVPP_CALIBRATIONBYTE_READ	,
		CMD_HVPP_ROM_PAGE_READ			,
		CMD_HVPP_PROG_CLOCK_SET			,
	}
	...
	public partial class CMcuFuncAVR8BitsHVPP  
	{
		...
		public override int CMcuFunc_OpenConnect(RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					...
					{
						this.mMsgText = "HVPP编程：接口打开成功!";
					}
					else
					{
						_return = 1;
						this.mMsgText = "HVPP编程：接口打开命令校验错误!";
					}
				}
				else
				{					
		...
		public override int CMcuFunc_CloseConnect( RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						this.mMsgText = "HVPP编程：接口关闭成功!";
					}
					else
					{
						_return = 1;
						this.mMsgText = "HVPP编程：接口关闭命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_EraseChip(RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						this.mMsgText = "HVPP编程：擦除成功!";
					}
					else
					{
						_return = 1;
						this.mMsgText = "HVPP编程：擦除命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_ReadChipFlash(ref byte[] chipFlash, RichTextBox msg, ToolStripLabel workState = null, ToolStripLabel workTime = null, ToolStripProgressBar workBar = null, string str = "读取Flash")
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				{
					//---大数据长度数据传输
					cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_FLASH_PAGE_READ, 0x00,0x00,0x00,0x00,0x00 };
					length += 1;
				}
				else
				{
					//---小数据包长度
					cmd= new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_FLASH_PAGE_READ, 0x00, 0x00, 0x00, 0x00 };
				}
				//---计算数据包的大小
				...
				for (int i=0;i<packageMaxNum;i++)
				{
					...
					{
						//---大数据长度数据传输
						cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_FLASH_PAGE_READ,(byte)(addr>>16) , (byte)(addr>>8), (byte)(addr), (byte)(length>>8), (byte)(length) };
					}
					else
					{
						//---小数据包长度
						cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_FLASH_PAGE_READ, (byte)(addr >> 16), (byte)(addr >> 8), (byte)(addr), (byte)(length) };
					}
					//---发送并读取命令
					_return = this.mCCOMM.SendCmdAndReadResponse(cmd, ref res);
					//---校验结果
					if (_return == 0)
					{
						...
						{
							_return = 1;
							this.mMsgText = "HVPP编程：Flash读取命令校验错误!";
							//---退出循环
							break;
						}
					}
				...
				if (_return==0)
				{
					this.mMsgText = "HVPP编程：Flash读取成功!";
		...
		public override int CMcuFunc_WriteChipFlash(byte[] chipFlash, RichTextBox msg, bool isAuto = false, ToolStripLabel  workState=null, ToolStripLabel workTime=null, ToolStripProgressBar workBar=null, string str = "编程Flash")
		{
			...
			{	
				...
				if (_return == 0)
				{
					...
					if (this.mCCOMM.mPerPackageMaxSize > 0xFF)
					{
						cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_FLASH_PAGE_WRITE, 0x00, 0x00, 0x00, 0x00, 0x00 };
						length += 1;
					}
					else
					{
						cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_FLASH_PAGE_WRITE, 0x00, 0x00, 0x00, 0x00 };
					}
					//---计算数据包的大小
					packageMaxSize = this.mCCOMM.mPerPackageMaxSize - length - cmd.Length;
					//---数据长度必须是偶数
						...
						if (CGenFuncEqual.GenFuncEqual(CGenFuncUnPackage.GenFuncUnPackage(flash[i]))==false)
						{
							for (int j = 0; j < flash[i].Length; j++)
							{
								...
								if (_return == 0)
								{
									...
									{
										_return = 1;
										this.mMsgText = "HVPP编程：Flash编程命令校验错误!";
										//---退出循环
										break;
									}
								}
					...
					if (_return == 0)
					{
						this.mMsgText = "HVPP编程：Flash编程成功!";
						TimeSpan timeSpan = DateTime.Now - nowTime;
						if (workTime!=null)
						{
							workTime.Text = timeSpan.Hours.ToString("#00")+":"+ timeSpan.Minutes.ToString("#00") + ":"+ timeSpan.Seconds.ToString("#00");
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						...
						{
							this.mMsgText = "HVPP编程：Flash为空!";
						}
						else
						{
							this.mMsgText = "HVPP编程：Flash不为空!";
							//---弹出窗体
					...
					{
						_return = 1;
						this.mMsgText = "HVPP编程：Flash查空命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_ReadChipEeprom(ref byte[] chipEeprom, RichTextBox msg, ToolStripLabel workState = null, ToolStripLabel workTime = null, ToolStripProgressBar workBar = null,string str="读取Eeprom")
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				{
					//---大数据长度数据传输
					cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_EEPROM_PAGE_READ, 0x00, 0x00, 0x00, 0x00};
					length += 1;
				}
				else
				{
					//---小数据包长度
					cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_EEPROM_PAGE_READ, 0x00, 0x00, 0x00 };
				}
				//---计算数据包的大小
				packageMaxSize = this.mCCOMM.mPerPackageMaxSize - length - cmd.Length;
				//---数据长度必须是偶数
				...
				for (int i = 0; i < packageMaxNum; i++)
				{
					...
					{
						//---大数据长度数据传输
						cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_EEPROM_PAGE_READ, (byte)(addr >> 8), (byte)(addr), (byte)(length >> 8), (byte)(length) };
					}
					else
					{
						//---小数据包长度
						cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_EEPROM_PAGE_READ, (byte)(addr >> 8), (byte)(addr), (byte)(length) };
					}
					//---发送并读取命令
					_return = this.mCCOMM.SendCmdAndReadResponse(cmd, ref res);
					//---校验结果
					if (_return == 0)
					{
						...
						{
							_return = 1;
							this.mMsgText = "HVPP编程：Eeprom读取命令校验错误!";
							//---退出循环
							break;
						}
					}
				...
				if (_return == 0)
				{
					this.mMsgText = "HVPP编程：Eeprom读取成功!";
					TimeSpan timeSpan = DateTime.Now - nowTime;
					if (workTime != null)
					{
						workTime.Text = timeSpan.Hours.ToString("#00") + ":" + timeSpan.Minutes.ToString("#00") + ":" + timeSpan.Seconds.ToString("#00");
		...
		public override int CMcuFunc_WriteChipEeprom(byte[] chipEeprom, RichTextBox msg, bool isAuto = false, ToolStripLabel workState = null, ToolStripLabel workTime = null, ToolStripProgressBar workBar = null,string str= "编程Eeprom")
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					...
					if (this.mCCOMM.mPerPackageMaxSize > 0xFF)
					{
						cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_EEPROM_PAGE_WRITE, 0x00, 0x00, 0x00, 0x00 };
						length += 1;
					}
					else
					{
						cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_EEPROM_PAGE_WRITE, 0x00, 0x00, 0x00 };
					}
					//---计算数据包的大小
					packageMaxSize = this.mCCOMM.mPerPackageMaxSize - length - cmd.Length;
					//---数据长度必须是偶数
					...
					for (int i = 0; i < eeprom.Length; i++)
					{
						...
						if (_return == 0)
						{
							...
							{
								_return = 1;
								this.mMsgText = "HVPP编程：Eeprom编程命令校验错误!";
								//---退出循环
								break;
							}
						}
					...
					if (_return == 0)
					{
						this.mMsgText = "HVPP编程：Eeprom编程成功!";
						TimeSpan timeSpan = DateTime.Now - nowTime;
						if (workTime != null)
						{
							workTime.Text = timeSpan.Hours.ToString("#00") + ":" + timeSpan.Minutes.ToString("#00") + ":" + timeSpan.Seconds.ToString("#00");
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						if (this.mCCOMM.mReceData.mArray[this.mCCOMM.mReceData.mIndexOffset]==0x00)
						{
							this.mMsgText = "HVPP编程：Eeprom为空!";
						}
						else
						{
							this.mMsgText = "HVPP编程：Eeprom不为空!";
							//---弹出窗体
					...
					{
						_return = 1;
						this.mMsgText = "HVPP编程：Eeprom查空命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_ReadChipFuse(ref byte[] chipFuse, RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					...
					{
						_return = 1;
						this.mMsgText = "HVPP编程：熔丝位读取命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_ReadChipLock(ref byte chipLock, RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					...
					{
						_return = 1;
						this.mMsgText = "HVPP编程：加密位读取命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_WriteChipFuse(byte[] chipFuse, RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				{
					cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_FUSE_WRITE,0x01, chipFuse[0], chipFuse[1], chipFuse[2] };
				}
				else
				{
					cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_FUSE_WRITE,0x00, chipFuse[0], chipFuse[1]};
				}				
				//---读取命令
				byte[] res = null;
				//---发送并读取命令
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						this.mMsgText = "HVPP编程：熔丝位写入成功!";
					}
					else
					{
						_return = 1;
						this.mMsgText = "HVPP编程：熔丝位写入命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_WriteChipLock(byte chipLock, RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						this.mMsgText = "HVPP编程：加密位写入成功!";
					}
					else
					{
						_return = 1;
						this.mMsgText = "HVPP编程：加密位写入命令校验错误!";
					}
				}
				else
				{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						...
						{
							this.mMsgText = "HVPP编程：ChipID读取成功!";
							//---申请缓存区
						...
						{
							this.mMsgText = "HVPP编程：ChipID读取成功，但数据不完整!";
							_return = 1;
						}
					}
					else
					{
						_return = 2;
						this.mMsgText = "HVPP编程：ChipID读取命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_ReadChipCalibration(ref byte[] chipCalibration, RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					...
					{
						_return = 1;
						this.mMsgText = "HVPP编程：校准字读取命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_ReadChipRom(ref byte[] chipRom, RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				{
					//---大数据长度数据传输
					cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_ROM_PAGE_READ, 0x00, 0x00, 0x00 };
					length += 1;
				}
				else
				{
					//---小数据包长度
					cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_ROM_PAGE_READ, 0x00, 0x00 };
				}
				//---计算数据包的大小
				packageMaxSize = this.mCCOMM.mPerPackageMaxSize - length - cmd.Length;
				//---数据长度必须是偶数
				...
				for (int i = 0; i < packageMaxNum; i++)
				{
					...
					{
						//---大数据长度数据传输
						cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_ROM_PAGE_READ, (byte)(addr), (byte)(length >> 8), (byte)(length) };
					}
					else
					{
						//---小数据包长度
						cmd = new byte[] { (byte)CMCUFUNC_CMD_HVPP.CMD_HVPP_ROM_PAGE_READ, (byte)(addr), (byte)(length) };
					}
					//---发送并读取命令
					_return = this.mCCOMM.SendCmdAndReadResponse(cmd, ref res);
					//---校验结果
					if (_return == 0)
					{
						...
						{
							_return = 1;
							this.mMsgText = "HVPP编程：ROM读取命令校验错误!";
							//---退出循环
							break;
						}
					}
		...
		public override int CMcuFunc_SetProgClock(byte chipClock, RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						this.mMsgText = "HVPP编程：编程时钟设置成功!";
					}
					else
					{
						_return = 2;
						this.mMsgText = "HVPP编程：编程时钟设置命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_EnableEepromPageMode(RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						this.mMsgText = "HVPP编程：EEPROM页编程模式打开!";
					}
					else
					{
						_return = 1;
						this.mMsgText = "HVPP编程：EEPROM页编程打开命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_DisableEepromPageMode(RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						this.mMsgText = "HVPP编程：EEPROM页编程模式关闭!";
					}
					else
					{
						_return = 1;
						this.mMsgText = "HVPP编程：EEPROM页编程关闭命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_WriteChipPower(int chipPWR, RichTextBox msg, bool isOpen = true)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					if (this.mCCOMM.mReceCheckPass)
					{
						this.mMsgText = "HVPP编程：供电电压设置成功!";
					}
					else
					{
						_return = 2;
						this.mMsgText = "HVPP编程：供电电压设置命令校验错误!";
					}
				}
				else
				{
		...
		public override int CMcuFunc_ReadChipPower(ref int chipPWR, RichTextBox msg)
		{
			...
			if ((this.mCCOMM != null) && (this.mCCOMM.mCOMMOpen == true))
			{
				...
				if (_return == 0)
				{
					...
					{
						_return = 2;
						this.mMsgText = "HVPP编程：供电电压读取命令校验错误!";
					}
				}
				else
				{
