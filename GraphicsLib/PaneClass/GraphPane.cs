...
namespace TestAgent.GraphicsLib
{
    ...
    public class GraphPane : PaneBase, ICloneable, ISerializable
    {
        ...
        /// <summary>Private field instance of the <see cref="ZYAxisList"/> class.  Use the
        /// public property <see cref="GraphPane.YAxisList"/> to access this class.</summary>
        private YAxisList _yAxisList;
        /// <summary>Private field instance of the <see cref="Y2AxisList"/> class.  Use the
        ...
        /// 标记数组
        /// </summary>
        private MarkObj[] _marks = new MarkObj[4];
        /// <summary>
        /// 目前活动的Mark
        /// </summary>
        private int _activeMarkIndex = -1;
        /// <summary>
        /// 使能Mark变量
        /// </summary>
        private bool _enableMark = false;
        /// <summary>
        /// 显示Mark
        /// </summary>
        ...
        public new struct Default
        {
            ...
            /// candidate.
            /// </summary>
            public static double NearestTol = 10.0;
        }
        #endregion 默认值
        ...
        public CurveItem this[int index]
        {
            get
            {
                if (index > -1 && index < this.CurveList.Count)
                    return this.CurveList[index];
                else
                    return null;
            }
        }
        ...
        /// 标记数组读写属性
        /// </summary>
        public MarkObj[] Marks { get { return this._marks; } set { this._marks = value; } }
        /// <summary>
        /// Mark的数量读写属性
        /// </summary>
        public int MarkCount
        {
            get { return this._marks.Length; }
            set
            {
                if (value > 0 && value < 10)
                    Array.Resize<MarkObj>(ref this._marks, value);
            }
        }
        /// <summary>
        /// true:使能Mark
        /// </summary>
        public bool EnableMark { get { return this._enableMark; } set { this._enableMark = value; } }
        /// <summary>
        /// true:显示Mark
        /// </summary>
        public bool ShowMark { get { return this._showMark; } set { this._showMark = value; } }
        #endregion 一般属性
            ...
            : base(title, rect)
        {
            ...
            // 加入Mark
            Color[] colors = new Color[5] { Color.Red, Color.Orange, Color.Green, Color.Blue, Color.Gray };
            for (int i = 0; i < this._marks.Length; i++)
            {
                this._marks[i] = new MarkObj();
                this._marks[i].Location.X = this._chart.Rect.X;
                this._marks[i].Location.Y = this._chart.Rect.Y;
                this._marks[i].Line.Style = (DashStyle)(i + 1);
                this._marks[i].Line.Color = colors[i];
            }
            this._graphObjList.AddRange(this._marks);
        }
        /// <summary>
        ...
        /// 用给定的曲线点构造本类对象
        /// </summary>
        /// <param name="title">绘图框标题</param>
        /// <param name="xTitle">X方向标题</param>
        /// <param name="yTitle">Y方向标题</param>
        /// <param name="label">曲线标签</param>
        /// <param name="points">数据点Point数组</param>
        public GraphPane(string title, string xTitle, string yTitle, string label, Point[] points)
            : this(new RectangleF(0, 0, 500, 375), title, xTitle, yTitle)
        {
            this.AddCurve(label, points);
        }
        /// <summary>
        /// 用给定的曲线点构造本类对象
        /// </summary>
        ...
        /// <param name="yTitle">Y方向标题</param>
        /// <param name="label">曲线标签</param>
        /// <param name="points">数据点PointF数组</param>
        public GraphPane(string title, string xTitle, string yTitle, string label, PointF[] points)
            : this(new RectangleF(0, 0, 500, 375), title, xTitle, yTitle)
        {
            this.AddCurve(label, points);
        }
        ...
        /// <param name="yTitle">Y方向标题</param>
        /// <param name="label">曲线标签</param>
        /// <param name="x">x方向数据数组</param>
        /// <param name="y">y方向数据数组</param>
        public GraphPane(string title, string xTitle, string yTitle, string label, double[] x, double[] y)
            : this(new RectangleF(0, 0, 500, 375), title, xTitle, yTitle)
        {
            this.AddCurve(label, x, y);
        }
         */

        public GraphPane(string title, string xTitle, string yTitle)
            : this(new RectangleF(0, 0, 500, 375), title, xTitle, yTitle)
        {
        }
        ...
        public void AxisChange(Graphics g)
        {
            ...
            double max = this._chart.Rect.Height < this._chart.Rect.Width ? this._chart.Rect.Width : this._chart.Rect.Height;
            GraphPane.Default.NearestTol = max / 10;
        }
        ...
        public override void Draw(Graphics g)
        {
            ...
            // 如果绘图区太小，不绘制图象--do a sanity check on the ChartRect
            if (_chart._rect.Width < 1 || _chart._rect.Height < 1)
                return;

            //for (int i = 0; i < this._marks.Length; i++)
            //{
            //    if (!_chart._rect.Contains(this._marks[i].Location.TopLeft))
            //    {
            //        this._marks[i].Location.X = _chart._rect.X;
            //        this._marks[i].Location.Y = _chart._rect.Y;
            //    }
            //}
            ...
            this._chart.Border.Draw(g, this, scaleFactor, _chart._rect);
        ...
        private void AddToCurveList(CurveItem item)
        {
            this._curveList.Add(item);
            if (this._curveList.Count == 1 && item != null && item.Points.Count > 0)
            {
                for (int i = 0; i < this._marks.Length; i++)
                {
                    this._marks[i].Location.X = item.Points[0].X;
                    this._marks[i].Location.Y = item.Points[0].Y;
                }
            }
        }
        ...
        public LineItem AddCurve(string label, IPointList points, Color color, SymbolType symbolType, float linwidth)
        {
            LineItem curve = new LineItem(label, points, color, symbolType, linwidth);
            this.AddToCurveList(curve);
        ...
        public LineItem AddCurve(string label, double[] x, double[] y, Color color, SymbolType symbolType, float linwidth)
        {
            LineItem curve = new LineItem(label, x, y, color, symbolType, linwidth);
            this.AddToCurveList(curve);

            return curve;
        }
        /// <summary>
        /// 用给定的Point数组和指定的颜色，数据点标识和线宽构建本对象
        /// </summary>
        /// <param name="label">图例标识</param>
        /// <param name="points">Point数组</param>
        /// <param name="color">颜色</param>
        /// <param name="symbolType">数据点标识符号</param>
        /// <param name="lineWidth">线宽</param>
        /// <returns>加入的曲线对象</returns>
        public LineItem AddCurve(string label, Point[] points, Color color, SymbolType symbolType, float lineWidth)
        {
            LineItem curve = new LineItem(label, points, color, symbolType, lineWidth);
            this.AddToCurveList(curve);

            return curve;
        }
        /// <summary>
        /// 用给定的PointF数组和指定的颜色，数据点标识和线宽构建本对象
        /// </summary>
        /// <param name="label">图例标识</param>
        ...
        /// <param name="lineWidth">线宽</param>
        /// <returns>加入的曲线对象</returns>
        public LineItem AddCurve(string label, PointF[] points, Color color, SymbolType symbolType, float lineWidth)
        {
            LineItem curve = new LineItem(label, points, color, symbolType, lineWidth);
            this.AddToCurveList(curve);
        ...
        /// <param name="points">A <see cref="IPointList"/> of double precision value pairs that define
        /// the X and Y values for this curve</param>
        /// <param name="color">The color to used for the curve line, symbols, etc.</param>
        /// <param name="symbolType">数据点标识类型</param>
        /// <returns>A <see cref="CurveItem"/> class for the newly created curve.
        /// This can then be used to access all of the curve properties that
        /// are not defined as arguments to the
        ...
        /// <param name="symbolType">数据点标识符号</param>
        /// <returns>加入的曲线对象</returns>
        public LineItem AddCurve(string label, Point[] points, Color color, SymbolType symbolType)
        {
            LineItem curve = new LineItem(label, points, color, symbolType);
            this.AddToCurveList(curve);
        ...
        /// <param name="symbolType">数据点标识符号</param>
        /// <returns>加入的曲线对象</returns>
        public LineItem AddCurve(string label, PointF[] points, Color color, SymbolType symbolType)
        {
            LineItem curve = new LineItem(label, points, color, symbolType);
            this.AddToCurveList(curve);
        ...
        // /// <param name="color">颜色</param>
        // /// <returns>加入的曲线对象</returns>
        // public LineItem AddCurve(string label, Point[] points, Color color)
        // {
        //     LineItem curve = new LineItem(label, points, color,SymbolType.Default);
        //     this.AddToCurveList(curve);
        ...
        // /// <param name="color">颜色</param>
        // /// <returns>加入的曲线对象</returns>
        // public LineItem AddCurve(string label, PointF[] points, Color color)
        // {
        //     LineItem curve = new LineItem(label, points, color,SymbolType.Default);
        //     this.AddToCurveList(curve);
        ...
        {
            LineItem curve = new LineItem(label, points, DefaultValue.GetDefaultColor(this._curveList.Count),
                                            DefaultValue.GetDefaultSymbolType(this._curveList.Count));
            this.AddToCurveList(curve);
        ...
        {
            LineItem curve = new LineItem(label, x, y, DefaultValue.GetDefaultColor(this._curveList.Count),
                                            DefaultValue.GetDefaultSymbolType(this._curveList.Count));
            this.AddToCurveList(curve);
        ...
        #region 增加条形曲线
        #region Error Bar误差条，标准偏差条绘制---序号 X，Y，Base三组值
        /// <summary>
        ...
        #endregion 误差条，标准偏差条绘制

        #region OHLC-棒型图 open-high-low-close
        /// 开市价（open）：垂直线底部左边的小水平线就是开市价
        /// 最高价（high）:垂直线顶部就是该时段的最高价
        /// 最低价（low）：垂直线底部就是该时段的最高价
        ///关市价（close）：垂直线顶部右边的小水平线就是开市价
        ///
        /// <summary>
        ...
        #region High-Low 高低图--需要 高-低-基础 三点数据
        /// 
        /// <summary>
        /// Add a "" bar type curve (<see cref="HiLowBarItem"/> object) to the plot with
        /// the given data points (double arrays) and properties.
        /// This is simplified way to add curves without knowledge of the
        /// <see cref="CurveList"/> class.  An alternative is to use
        /// the <see cref="CurveList" /> Add() method.
        ...
        /// 更新曲线数据
        /// </summary>
        /// <param name="item">曲线</param>
        /// <param name="points">数据点</param>
        public void UpdateWith(CurveItem item, PointPairList points)
        {
            item.Clear();
            item.UpdatePointsWith(points);
            base.CreatDataChangeEvent(item,points);
        }
        #endregion 更新曲线数据
        ...
        /// <summary>
        /// 在左侧增加一个Y轴<see cref="YAxis" />到轴列表中
        ...
        /// <summary>
        /// 在右侧增加一个Y轴<see cref="YAxis" />到轴列表中
            ...
            out object nearestObj, out int index)
        {
            ...
            if (AxisRangesValid())
            {
                ...
                RectangleF tmpRect;
                GraphObj saveGraphItem = null;  // 查找到的图形对象
                int saveIndex = -1;
                ...
                // 查看点是否在图形上
                // 如果在，先存下来，然后再查找有没有覆盖在它上面的更准确的对象
        ...
        public ArrayList FindNeareObjects(PointF mousePt, Graphics g)
        {
            ...
            if (AxisRangesValid())
            {
                ...
                // 查看点是否在图例上
                _return.AddRange(this.Legend.FindObjsOnPoint(mousePt, this, scaleFactor));
                ...
                for (int yIndex = 0; yIndex < _yAxisList.Count; yIndex++)
                {
                    ...
                    if (width > 0)
                    {
                        ...
                        if (tmpRect.Contains(mousePt))
                        {
                            _return.Add(this._yAxisList[yIndex]);
                        }
                        left -= width;
                    }
                }
                ...
                for (int yIndex = 0; yIndex < _y2AxisList.Count; yIndex++)
                {
                    ...
                    if (width > 0)
                    {
                        ...
                        if (tmpRect.Contains(mousePt))
                        {
                            _return.Add(this._y2AxisList[yIndex]);
                        }
        ...
        /// <param name="y"></param>
        /// <returns></returns>
        private int FindNeastMark(double x, double y)
        {
            int _return = 0;
            double dis0 = (x - this._marks[0].Location.X) * (x - this._marks[0].Location.X)
                                + (y - this._marks[0].Location.Y) * (y - this._marks[0].Location.Y);
            for (int i = 1; i < this._marks.Length; i++)
            {
                double dis = (x - this._marks[i].Location.X) * (x - this._marks[i].Location.X)
                                + (y - this._marks[i].Location.Y) * (y - this._marks[i].Location.Y);
                if (dis < dis0)
                {
                    dis0 = dis;
                    _return = i;
        ...
        /// <param name="mouseModer"></param>
        /// <param name="g"></param>
        /// <returns></returns>
        public Location ShowSelectedObj(Point mouseLocation, int mouseModer, Graphics g)
        {
            if (this._chart.Rect.Contains(new PointF(mouseLocation.X, mouseLocation.Y)))
            {
                double x, y;
                this.ReverseTransform(new PointF(mouseLocation.X, mouseLocation.Y), out x, out y);

                if (mouseModer == 0)
                    this._activeMarkIndex = this.FindNeastMark(x, y);

                if (this._activeMarkIndex > -1)
                {
                    object selectedObj = null;
                    int index = -1;
                    if (this.FindNearestObject(new PointF(mouseLocation.X, mouseLocation.Y), g, out selectedObj, out index))
                    {
                        if (selectedObj is CurveItem)
                        {
                            if (index > -1)
                            {
                                this._marks[this._activeMarkIndex].TargetCurve = (CurveItem)selectedObj;
                                this._marks[this._activeMarkIndex].DataIndex = index;
                                this._marks[this._activeMarkIndex].Location.X = ((CurveItem)selectedObj).Points[index].X;
                                this._marks[this._activeMarkIndex].Location.Y = ((CurveItem)selectedObj).Points[index].Y;
                            }
                            else
                            {
                                this._marks[this._activeMarkIndex].Location.X = ((CurveItem)selectedObj).Points[0].X;
                                this._marks[this._activeMarkIndex].Location.Y = ((CurveItem)selectedObj).Points[0].Y;
                            }
                        }
                    }
                    else
                    {
                        this._marks[this._activeMarkIndex].Location.X = x;
                        this._marks[this._activeMarkIndex].Location.Y = y;
                    }
                    return this._marks[this._activeMarkIndex].Location;
                }
            }
            return null;
        }
        ...
        {
            return this.ShowSelectedObj(mouseLocation, 0, g);
        }

        public Location MouseMove(Point mouseLocation, Graphics g)
        {
            return this.ShowSelectedObj(mouseLocation, 1, g);
        }

        public Location MouseUp(Point mouseLocation, Graphics g)
        {
            Location lo =  this.ShowSelectedObj(mouseLocation, 2, g);
            this._activeMarkIndex = -1;
            return lo;
        }
        ...
        {
            return new FormGraphPaneParamEdit(this);
        }
        #endregion 参数编辑界面
    }
}
